...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Create', href: '/sales/payment-methods/create' },
];
...
export default function PaymentMethodCreate() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.post('/sales/payment-methods', submitData, {
            onSuccess: () => {
                    ...
                    <CardHeader>
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <CardDescription>Add a new payment method with fee configuration</CardDescription>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">Enable this payment method for use</p>
                                    </div>
                                    <Switch
                                        id="is_active"
                            ...
                            <div className="space-y-4 border-t pt-4">
                                ...
                                {formData.fee_type !== 'none' && (
                                    <div>
                                        <Label htmlFor="fee_value">Fee Value {formData.fee_type === 'percentage' ? '(%)' : '(IDR)'}</Label>
                                        <Input
                                            id="fee_value"
                                            type="number"
                                        ...
                                        {formData.fee_type === 'percentage' && formData.fee_value > 0 && (
                                            <p className="mt-1 text-sm text-muted-foreground">
                                                Example fee for {formatCurrency(100000)}: {formatCurrency(100000 * (formData.fee_value / 100))}
                                            </p>
                                        )}
                                        {formData.fee_type === 'fixed' && formData.fee_value > 0 && (
                                            <p className="mt-1 text-sm text-muted-foreground">
                                                Fixed fee: {formatCurrency(formData.fee_value)} per transaction
                                            </p>
                                        )}
